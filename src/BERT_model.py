...
def BERT_model(data):
    ...
    # A tensor is a fundamental data structure representing multi-dimensional arrays. Tensors can be scalars,
    ...
    #  Initiates a loop that iterates over the specified number of epochs
    for epoch in range(epochs):
        # Sets the model to training mode
        model.train()

        # Sets the training loss to zero at the beginning of each epoch
        train_loss = 0

        # Iterates through batches in the TRAINING dataloader
        for batch in train_dataloader:
            # Unpacks the batch into inputs, masks, and labels.
            ...
            # Clears the gradients of the model parameters. Gradients accumulate by default, and this step is essential
            # before computing the gradients for a new batch
            model.zero_grad()

            # Obtains model predictions (logits) for the given inputs and masks.
            logits = model(batch_inputs, batch_masks)

            # Computes the loss between the model predictions (logits) and the actual labels
            loss = loss_function(logits, batch_labels)

            # Adds the current batch loss to the running total for the epoch
            train_loss += loss.item()

            # Backpropagates the gradients, clips gradients to prevent exploding gradients, updates model parameters
            # using the optimizer, and adjusts the learning rate if a learning rate scheduler is used.
            loss.backward()
        ...
        # Initializes variables to store the testing loss and accuracy for the current epoch.
        test_loss = 0
        test_accuracy = 0

        # Iterates through batches in the TESTING dataloader
        for batch in test_dataloader:
            ...
            # computes and accumulates the accuracy of the model's predictions for a testing batch, expressing it as a
            # percentage and updating the overall accuracy for the entire testing dataset.
